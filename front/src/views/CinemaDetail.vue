...
const loadSchedules = async () => {
  ...
  try {
    // 传递 cinemaId 参数获取指定影院的场次
    const response = await fetch(`${API_BASE_URL}/admin/schedules?cinemaId=${cinemaId}`)
    const data = await response.json()
    console.log('当前选中日期:', selectedDate.value)
    console.log('后端返回的场次总数:', data.data?.length || 0)
    
    if (data.success) {
      // 筛选日期匹配的场次（处理时区问题）
      const filteredSchedules = data.data.filter(s => {
        if (s.status !== 'available') return false
        
        // 直接比较日期字符串（假设后端返回的格式是 yyyy-MM-dd HH:mm:ss）
