...
const loadHalls = async () => {
  ...
  try {
    const response = await fetch(`${API_BASE_URL}/halls?cinemaId=${selectedCinemaId.value}`)
    const data = await response.json()
    if (data.success) {
...
const loadSchedules = async () => {
  ...
  try {
    const response = await fetch(`${API_BASE_URL}/admin/schedules?cinemaId=${selectedCinemaId.value}`)
    const data = await response.json()
    if (data.success) {
...
const handleSubmit = async () => {
  ...
  try {
    ...
    if (data.success) {
      router.push({
        path: '/payment',
        query: { orderId: data.data.id.toString() }
      })
    } else {
