...
const loadOrderInfo = async () => {
  ...
  try {
    const response = await request.get('/' + 'orders/${orderId.value}`')
    ...
    if (data.code === 200 && data.data) {
      ...
      // 获取场次信息
      const scheduleResponse = await request.get('/' + 'admin/schedules`')
      ...
      if (scheduledata.code === 200) {
        ...
        if (schedule) {
          ...
          // 获取电影信息
          const movieResponse = await request.get('/' + 'movies/${schedule.movieId}`')
          ...
          // 获取影院信息
          const cinemaResponse = await request.get('/' + 'cinemas/${schedule.cinemaId}`')
          const cinemaData = await cinemaResponse.json()
          if (cinemadata.code === 200) {
            orderInfo.value.cinemaName = cinemaData.data.name
          }
          
          // 获取放映厅信息
          const hallResponse = await request.get('/' + 'halls/${schedule.hallId}`')
          const hallData = await hallResponse.json()
          if (halldata.code === 200) {
      ...
      // 获取座位信息
      const orderSeatResponse = await request.get('/' + 'order-seats/order/${orderId.value}`')
      const orderSeatData = await orderSeatResponse.json()
      if (orderSeatdata.code === 200) {
        const seatIds = orderSeatData.data.map(os => os.seatId)
        const seats = []
        for (const seatId of seatIds) {
          const seatResponse = await request.get('/' + 'seats/${seatId}`')
...
const handlePay = async () => {
  ...
  try {
    await request.put(`/orders/${orderId.value}/pay`, {
...
const handleCancel = async () => {
  ...
  try {
    await request.put(`/orders/${orderId.value}/cancel`)
    alert('订单已取消')
    router.push('/')
  } catch (error) {
