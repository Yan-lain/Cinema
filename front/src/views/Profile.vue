...
const removeBrowseHistory = async (item) => {
  if (authStore.isAuthenticated && item.recordId) {
    try {
      const response = await fetch(`${API_BASE_URL}/browse/delete/${item.recordId}`, {
        method: 'DELETE'
      })
      const data = await response.json()
      if (data.success) {
...
const loadMyComments = async () => {
  if (authStore.isAuthenticated && authStore.user?.id) {
    try {
      const response = await fetch(`${API_BASE_URL}/comments/user/${authStore.user.id}`)
      const data = await response.json()
      if (data.success) {
        myComments.value = data.data
        commentCount.value = data.data.length
      }
    } catch (error) {
...
const deleteComment = async (commentId) => {
  ...
  try {
    const response = await fetch(`${API_BASE_URL}/comments/${commentId}?userId=${authStore.user.id}`, {
      method: 'DELETE'
    })
    const data = await response.json()
    if (data.success) {
...
const loadOrders = async () => {
  ...
  try {
    const response = await fetch(`${API_BASE_URL}/orders/user/${authStore.user.id}`)
    const data = await response.json()
    if (data.success) {
...
const loadUserInfo = async () => {
  ...
  try {
    const response = await fetch(`${API_BASE_URL}/auth/userinfo?userId=${authStore.user.id}`)
    const data = await response.json()
    if (data.success) {
