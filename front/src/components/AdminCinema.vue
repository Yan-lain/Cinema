...
const loadCinemas = async () => {
  try {
    let url = `${API_BASE_URL}/cinemas`
    if (searchQuery.value.trim()) {
      url += `/search?name=${encodeURIComponent(searchQuery.value)}`
    }
...
const saveCinema = async () => {
  try {
    const method = editingCinema.value ? 'PUT' : 'POST'
    const url = editingCinema.value
      ? `${API_BASE_URL}/cinemas/${editingCinema.value.id}`
