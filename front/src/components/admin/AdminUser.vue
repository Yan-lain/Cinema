 ...
 * 【涉及文件】src/components/admin/AdminUser.vue
 * 【潜在影响】无，只是修复导入语法错误
 */
...
const loadUsers = async () => {
  try {
    users.value = await request.get('/admin/users', { params: { role: 'user' } })
  } catch (error) {
...
const saveUser = async () => {
  try {
    ...
    if (editingUser.value) {
      await request.put(`admin/users/${editingUser.value.id}`, body)
    } else {
      await request.post('/admin/users', body)
    }
...
const resetPassword = async () => {
  ...
  try {
    await request.put(`admin/users/${resetUser.value.id}/password`, { password: newPassword.value })
    alert('密码重置成功')
    closeResetModal()
  } catch (error) {
