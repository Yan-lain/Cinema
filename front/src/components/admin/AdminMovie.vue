...
const loadMovies = async () => {
  try {
    // 响应拦截器已解包 ApiResponse，直接得到 data 字段内容（电影列表）
    movies.value = await request.get('/admin/movies')
  } catch (error) {
...
const addMovie = async () => {
  try {
    const data = await request.post('/admin/movies', formData.value)
...
const toggleStatus = async (movie) => {
  ...
  try {
    await request.put(`/admin/movies/${movie.id}/status`, { status: newStatus })
